...
export default function FlashcardsGenerator() {
  ...
  const handleSaveFlashcards = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "collections", selectedCollection.id, "flashcards"), {
        flashcards, 
  ...
  const generateFlashcardsFromGemini = async (text: string) => {
    ...
    try {
      ...
      const flashcardPairs = flashcardBlocks
        ...
        .filter(Boolean); // Remove any null or invalid blocks

      setFlashcards(flashcardPairs);
    } catch (error) {
      console.error("Error generating flashcards:", error);
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-2xl p-6 rounded-lg shadow-2xl bg-opacity-80 backdrop-blur-lg mt-5 ${
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mb-4">
          <textarea
            ...
            className="w-full p-4 mb-2 border-none rounded-lg bg-gray-800 text-gray-200"
            rows={4}
          />
          ...
          className={`py-2 px-4 bg-gray-800 text-white font-bold rounded-lg hover:bg-gray-700 transition duration-300 ml-2 mr-2 relative top-1 ${flashcards.length === 0?"cursor-not-allowed opacity-50":""}`}
          disabled={flashcards.length === 0}
        >
          <span className="flex justify-center items-center"><FaSave className="mr-2" /> Save Collection</span>
        </button>
        ...
        <div className="mt-6 flex flex-col items-center">
          {" "}
          {flashcards.length > 0 && (
            <div className="w-60 bg-gray-200 rounded-full h-2.5 mb-4 overflow-hidden">
              <div
          ...
          {/* Center flashcards and pagination */}
          {flashcards.length > 0 && (
            <div
              key={currentPage}
              className={`w-96 h-96 p-8 rounded-lg shadow-lg cursor-pointer relative flashcard 
                    ...
                    transform transition-transform duration-500 hover:scale-105`} // Added hover effect and theme-based styling
              onClick={() => toggleAnswer(currentPage)}
              title="Click to view answer"
            >
              <div className="front absolute inset-0 flex flex-col justify-center items-center p-6">
                {" "}
                {/* Adjusted padding and flexbox */}
